...
from mpl_toolkits.mplot3d import Axes3D  # czasem wymagane przez Matplotlib
...
SAVE_PLOTS = False          # ustaw True, jeśli chcesz zapisać wykresy do plików
OUTPUT_DIR = "outputs"      # folder na wykresy, jeśli SAVE_PLOTS=True
...
def load_data(path: str) -> pd.DataFrame:
    ...
    # Usunięcie kolumny technicznej (często powstaje przy zapisie CSV z indexem)
    if "Unnamed: 0" in df.columns:
        df = df.drop(columns=["Unnamed: 0"])

    # Upewniamy się, że dane są 0/1 (bool/int) – apriori/fpgrowth tego oczekują
    # Jeśli masz już 0/1, to nic nie popsuje.
...
def main():
    ...
    # Wykresy + interpretacja
    plot_top_items_support(frequent_apriori)
    plot_top_rules_lift(rules_apriori)     # <-- POPRAWIONY wykres 2 (etykiety lift)
